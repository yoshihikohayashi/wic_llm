...
class WiC_with_Bert(BertPreTrainedModel):
    ...
    #
    def __init__(self, config, wv_dim):
        ...
        if wv_dim==0:
            self.classifier = nn.Sequential(
                nn.Linear(config.hidden_size, 128), # 2023-10-04 20:28:00
            ...
            self.classifier = nn.Sequential(
                nn.Linear(config.hidden_size+config.hidden_size*2, 128), # 2023-10-04 20:28:00
                nn.ReLU(inplace=True),
                nn.Dropout(0.25),
                nn.Linear(128, config.num_labels), # 2023-10-04 20:41:47
            )
            ...
            self.classifier = nn.Sequential(
                nn.Linear(config.hidden_size+self.wv_dim*2, 128), # 2023-10-04 20:28:00
                nn.ReLU(inplace=True),
                nn.Dropout(0.25),
                nn.Linear(128, config.num_labels), # 2023-10-04 20:41:47
            )
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        bert_outputs = self.bert(input_ids, attention_mask=attention_mask,
                                        ...
                                        **kwargs)
        dropout_output = self.dropout(bert_outputs[1]) # bert_outputs[1] is pooled output
...
def main(verbose=True, save_db=True):
    ...
    arg_p.add_argument('--wv_dim', type=int, default=0, help='word vector dimensionality') # -1, 0, 16, 64, ....
    ...
    wic_with_bert_model = (WiC_with_Bert.from_pretrained(bert_model_name, config=wic_with_bert_config, wv_dim=args.wv_dim).to(device))
    #    
    ...
    if save_db:
        ...
        wic_cl_results_db = tinydb.TinyDB(wic_db_path + 'cl_config_1' + '.json') 
        #

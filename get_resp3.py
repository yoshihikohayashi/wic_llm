# ChatGPT に 比較・対照させ，結果を記述させる 2023-09-30 09:47:57
# gpt-4 対応追加，tinyDBではなく tsv 保存
# コードの整理・最適化必要 (もとは wic_ex3.py で無駄がいっぱいかと)
import pandas
import time, ray
import re
import openai
...
### prompt templates
compare_template_str = '''
Compare the core senses of \"{:s}\" in these two sentences and summarize the result in a short sentence.
1: {:s}
2: {:s}
'''
...
direct_template_str_ = '''
Determine whether the target word "{:s}" in the following sentences corresponds to identical meanings or not. 
Please respond with "Yes" or "No" and provide a brief sentence explaining the reasoning behind your choice.
1: {:s}
2: {:s}
'''
...
compare_template = prompt_tamplate('compare', compare_template_str)
...
### top
def get_resp(dataset='test', s=0, e=10, 
    ...
    if save_tsv:
        fname = './tsv/wic_desc_{ftype}_{prtype}_{model}.tsv'.format(ftype=dataset, prtype=pr_template.name, model=llm)

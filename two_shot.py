...
# two-shot prompt template 2023-09-10 13:13:38
two_shot_template = '''
...
Consider the following examples in decision.
[Example 1]
Target word: {word_}
c1: {c1_}
c2: {c2_}
...
two_shot_prompt = prompt_template('two-shot-default', two_shot_template)

###
def eval_wic_fs(adj='identical', 
                target_dataset='test', s=0, e=0,
                ref_dataset='dev',  
                pr_template=two_shot_prompt, 
    ...
    wic_db_target = tinydb.TinyDB(wic_db_path + 'wic_tspr_' + _db_name_ + '.json') 
...
def wic_(adj, target_id, word, pos, c1, c2, 
    ...
    example1, example2 =\
        select_examples(target_df, target_id, ref_df, word, c1, c2)
    word_, pos_, c1_, c2_, ans_ = example1
    word__, pos__, c1__, c2__, ans__ = example2
    #
    query = pr_template_body.format(adj=adj, word=word, c1=c1, c2=c2, 
                                    word_=word_, c1_=c1_, c2_=c2_, ans_=ans_,
                                    word__=word__, c1__=c1__, c2__=c2__, ans__=ans__)
...
#
def select_examples(target_df, target_id, ref_df, word, c1, c2, select_func=select_example_with_sim):
    ...
    if _sel_TF=='TF':
        cands_T = ref_df[(ref_df['label']=='T') & (ref_df['pos']==POS)]
        cands_F = ref_df[(ref_df['label']=='F') & (ref_df['pos']==POS)]
        cands = [cands_T, cands_F]
    else:
        _cands = ref_df[(ref_df['label']==_sel_TF) & (ref_df['pos']==POS)]
        cands = [_cands, _cands]
    print('# of candidates:', len(ref_df), 'records', flush=True)
    #
    if _sel_Dsim:
    ...
    #
    sel_pos_1 = select_func(cands[0], target_sim, 'T')
    sel_1 = cands[0].iloc[sel_pos_1]
    sel_pos_2 = select_func(cands[1], target_sim, 'F')
    sel_2 = cands[1].iloc[sel_pos_2]
    #
    print('> selected ids:', sel_1['id'], sel_2['id'])
    print('> selected words POS:', sel_1['word'], sel_2['word'], POS)
    #
    return [(sel_1['word'], sel_2['pos'], sel_1['c1'], sel_1['c2'], sel_1['label']),\
        (sel_2['word'], sel_2['pos'], sel_2['c1'], sel_2['c2'], sel_2['label'])]
